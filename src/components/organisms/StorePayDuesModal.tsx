...
const useDueForm = (
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: ["pay-store-dues", { storeId: store.id }],
    mutationFn: (values: FormValues) => payDues(values, store),
    onSuccess: () => {
      const t = toast({
      ...
      queryClient.invalidateQueries({
        queryKey: ["store-details", { storeId: store.id.toString() }],
      });
      queryClient.invalidateQueries({
        queryKey: ["store-transactions", { storeId: store.id }],
      });
      formik.resetForm();
      setOpen(false);
    },
...
const payDues = async (values: FormValues, store: Store) => {
  const res = await api.post(
    `/dashboards/stores/${store.id}/pay-to-store`,
    values
  );
  console.log(res);
  return res.data;
